...
@Component({
  ...
  template: `
    <section class="page-top">
      <div>
        <h1>Utilisateurs</h1>
        <p>Admin : gestion complète des comptes.</p>
      </div>
      ...
      <div class="entity-card" *ngFor="let user of users">
        <div class="entity-head">
          <div>
            ...
            <p class="entity-subtitle">{{ user.role }}</p>
          </div>
      ...
      <div class="modal-card">
        ...
        <form [formGroup]="form" (ngSubmit)="submit()">
          ...
          <div class="modal-actions">
            <button type="button" class="btn-secondary-pro" (click)="close()">Annuler</button>
...
export class UsersComponent implements OnInit {
  ...
  openCreate(): void {
    ...
    this.form.reset({ username: '', password: '', role: 'ROLE_STAFF', active: true });
    this.showModal = true;
  }

  openEdit(user: AdminUser): void {
    this.editingId = user.id;
    this.form.reset({ username: user.username, password: '', role: user.role, active: user.active });
    this.showModal = true;
  }
  ...
  submit(): void {
    ...
    const obs = this.editingId ? this.api.updateUser(this.editingId, p) : this.api.createUser(p);
    obs.subscribe(() => { this.close(); this.load(); });
  }
